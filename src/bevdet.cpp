...
void BEVDet::MallocDeviceMemory(){
    ...
    for(int i = 0; i < trt_engine->getNbIOTensors(); i++){
        // nvinfer1::Dims dim = trt_context->getBindingDimensions(i);
        // nvinfer1::Dims dim = trt_context->getTensorShape(i);
        ...
        // size *= dataTypeToSize(trt_engine->getTensorDataType(i));
...
// void print_dim(nvinfer1::Dims dim){
//     for(auto i = 0; i < dim.numDims; i++){  // Changed from nbDims to numDims
//         printf("%ld%c", dim.extent[i], i == dim.numDims - 1 ? '\n' : ' ');  // Changed from dim.d to dim.extent
//     }
// }
...
int BEVDet::InitEngine(const std::string &engine_file){
    ...
    for(auto i = 0; i < trt_engine->getNbIOTensors(); i++){
        ...
        // auto dim = trt_context->getTensorShape(i);
        auto dim = trt_context->getTensorShape(trt_engine->getIOTensorName(i));
        // auto name = trt_engine->getTensorName(i);
...
int BEVDet::DoInfer(const camsData& cam_data, std::vector<Box> &out_detections, float &cost_time,
    ...
    double fps = 1.0 / engine_t.count() + post_t.count() + infer_t.count();
    ...
    // printf("Detection %ld: Label = %d, Score = %.2f\n", i, out_detections[i].label, out_detections[i].score);
    // }
...
BEVDet::~BEVDet(){
    ...
    // trt_context->destroy();
    // trt_engine->destroy();
    delete trt_context;  // Replace trt_context->destroy()
    delete trt_engine;
}
